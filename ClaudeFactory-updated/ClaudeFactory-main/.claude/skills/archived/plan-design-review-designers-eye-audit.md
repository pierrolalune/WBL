...
1. Navigate to the target URL using `browser_navigate` with the target URL.
2. Take a full-page desktop screenshot using `browser_take_screenshot`. Save the returned screenshot to `design-reports/screenshots/first-impression.png`.
...
**Fonts in use** (capped at 500 elements to avoid timeout) — use `browser_evaluate` with expression:
```
JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).map(e => getComputedStyle(e).fontFamily))])
```

**Color palette in use** — use `browser_evaluate` with expression:
```
...
**Heading hierarchy** — use `browser_evaluate` with expression:
```
...
**Touch target audit** (find undersized interactive elements) — use `browser_evaluate` with expression:
```
...
**Performance baseline** — use `browser_evaluate` with expression:
```
...
1. **Navigate** to the page URL using `browser_navigate`.
2. **Take a screenshot** using `browser_take_screenshot`, then use `browser_snapshot` to get the accessibility tree for context.
3. **Responsive check:** Use `browser_resize` to set viewport width to 375 (mobile), then `browser_take_screenshot`. Repeat at 768 (tablet) and 1280 (desktop). Save all screenshots to `design-reports/screenshots/{page}-{width}.png`.
4. **Console errors:** Use `browser_console_messages` to check for errors.
5. **Performance:** Use `browser_evaluate` with expression `JSON.stringify(performance.getEntriesByType('navigation')[0])`.

### Auth Detection

After the first navigation, check if the URL changed to a login-like path using `browser_evaluate` with expression `window.location.href`.

If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Please handle authentication manually (e.g., log in via the browser) before continuing."
...
- `text-wrap: balance` or `text-pretty` on headings (check via `browser_evaluate` with expression `getComputedStyle(document.querySelector('h1')).textWrap`)
...
- `prefers-reduced-motion` respected (check via `browser_evaluate` with expression `matchMedia('(prefers-reduced-motion: reduce)').matches`)
...
1. Take a **before snapshot** using `browser_snapshot` to see the current state and available interactive elements.
2. Perform an action using `browser_click` with the appropriate element reference.
3. Take an **after snapshot** using `browser_snapshot` and compare it with the before snapshot to see what changed.
...
2. **Screenshots are evidence.** Every finding needs at least one screenshot. Use `browser_take_screenshot` for visual evidence and `browser_snapshot` for accessibility tree context.
3. **Be specific and actionable.** "Change X to Y because Z" — not "the spacing feels off."
4. **Never read source code.** Evaluate the rendered site, not the implementation. (Exception: offer to write DESIGN.md from extracted observations.)
5. **AI Slop detection is your superpower.** Most developers can't evaluate whether their site looks AI-generated. You can. Be direct about it.
6. **Quick wins matter.** Always include a "Quick Wins" section — the 3-5 highest-impact fixes that take <30 minutes each.
7. **Use `browser_snapshot` for tricky UIs.** The accessibility tree reveals interactive elements that may not be visually obvious.
...
11. **Never fix anything.** Find and document only. Do not read source code, edit files, or suggest code fixes. Your job is to report what could be better and suggest design improvements.
