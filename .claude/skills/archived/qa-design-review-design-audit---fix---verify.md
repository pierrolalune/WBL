...
2. Take a full-page desktop screenshot using `browser_take_screenshot` — save to `design-reports/screenshots/first-impression.png`.
...
Use `browser_evaluate` for each of the following:
...
**Performance baseline** — use `browser_evaluate` with the Performance API:
```js
JSON.stringify(performance.getEntriesByType('navigation').map(e => ({domContentLoaded: e.domContentLoadedEventEnd, load: e.loadEventEnd, ttfb: e.responseStart})))
```
...
2. Use `browser_snapshot` to capture an annotated accessibility snapshot — save to `design-reports/screenshots/{page}-annotated.png`.
3. **Responsive screenshots:** Use `browser_resize` at three widths (375, 768, 1280) and take a `browser_take_screenshot` at each — save to `design-reports/screenshots/{page}-mobile.png`, `{page}-tablet.png`, `{page}-desktop.png`.
4. Use `browser_console_messages` to check for errors.
5. Use `browser_evaluate` with the Performance API to check page performance.

### Auth Detection

After the first navigation, check if the URL changed to a login-like path using `browser_evaluate`:
```js
window.location.href
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser?"
...
- `text-wrap: balance` or `text-pretty` on headings (check via `browser_evaluate` with `getComputedStyle()`)
...
- `prefers-reduced-motion` respected (check via `browser_evaluate`: `matchMedia('(prefers-reduced-motion: reduce)').matches`)
...
1. Use `browser_snapshot` to capture the current state before interaction.
2. Use `browser_click` to perform the action (e.g., click a button or link by its ref).
3. Use `browser_snapshot` again to capture the state after interaction. Compare before and after snapshots to see what changed.
...
2. **Screenshots are evidence.** Every finding needs at least one screenshot. Use `browser_take_screenshot` to capture evidence.
...
7. **Use `browser_snapshot` for tricky UIs.** Captures the accessibility tree to find interactive elements that may not be visually obvious.
...
1. Use `browser_navigate` to go to the affected URL.
2. Use `browser_take_screenshot` — save to `design-reports/screenshots/finding-NNN-after.png`.
3. Use `browser_console_messages` to check for new errors.
4. Use `browser_snapshot` before and after to compare changes.
