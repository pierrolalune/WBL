# Product Discovery Workflow — New Features Only

Analyze the current project and surface **substantial new feature proposals** — entirely new capabilities, screens, integrations, or user flows that do not exist yet.
...
- **Strictly filter out**: anything that improves an existing feature rather than adding a new one. If a capability already exists in any form, it's an improvement, not a new feature — exclude it.
- Classify each opportunity: **New Feature / New Integration / New Screen or Flow / Major Architectural Enabler**
- Output: **Opportunity log** — structured list of new feature use cases with persona, problem, and business impact

## Phase 3: Feature Ideation

Generate and frame **new feature** proposals.
- For each opportunity in the log, formulate a concrete new feature proposal:
  - Name, description, target persona
  - What entirely new capability does this add to the product?
...
- **Final filter**: remove any proposal that is really just an enhancement to an existing feature. Each proposal must introduce something the product cannot do at all today.
...
Produce the final Product Discovery Report.

## Phase 6: Proposal Files & Index Updates

Create individual proposal files and update all discovery indexes.